...
func _physics_process(delta):
	if health <= 0:
		player_death()
		return
	var x_input = Input.get_axis("move_left_P2", "move_right_P2")
	var jump_pressed := Input.is_action_just_pressed("jump_P2") #or Input.is_action_just_pressed("jump_controller")
	var jump_released := Input.is_action_just_released("jump_P2")# or Input.is_action_just_released("jump_controller")
	var dash_pressed := Input.is_action_just_pressed("dash_P2")# or Input.is_action_just_pressed("dash_controller")
	
	if Input.is_action_just_pressed("interact_P2"): # or Input.is_action_just_pressed("interact_controller"):
	...
	# Melee attack input
	if Input.is_action_just_pressed("attack_P2"): #or Input.is_action_just_pressed("attack_controller"):
		_try_attack()
